#Проверяется, загружены ли апдейты и сколько 
#Файл с IP
...
Function Get-WUList
{
	...
	.EXAMPLE
		...
		------------ ------ --          ---- -----
		KOMPUTER     ------ KB976002  102 KB Aktualizacja firmy Microsoft z ekranem wybierania przegl№darki dla uїytkownikуw...
		KOMPUTER     ------ KB971033    1 MB Aktualizacja dla systemu Windows 7 dla systemуw opartych na procesorach x64 (KB...
		KOMPUTER     ------ KB2533552   9 MB Aktualizacja systemu Windows 7 dla komputerуw z procesorami x64 (KB2533552)
		KOMPUTER     ------ KB982861   37 MB Windows Internet Explorer 9 dla systemu Windows 7 - wersja dla systemуw opartyc...
		KOMPUTER     D----- KB982670   48 MB Program Microsoft .NET Framework 4 Client Profile w systemie Windows 7 dla syst...
		KOMPUTER     ---H-- KB890830    1 MB Narzкdzie Windows do usuwania zіoњliwego oprogramowania dla komputerуw z proces...
	...
	.EXAMPLE
		...
		ComputerName Status KB          Size Title
		------------ ------ --          ---- -----
		G1           DI--U- KB2345886 605 KB Aktualizacja dla systemu Windows 7 dla systemуw opartych na procesorach x64 (KB...
		G1           DI--U- KB2641690  67 KB Aktualizacja systemu Windows 7 dla komputerуw z procesorami x64 (KB2641690)
	...
	.EXAMPLE
		...
		ComputerName Status KB          Size Title
		------------ ------ --          ---- -----
		KOMPUTER     ---H-- KB982861   37 MB Windows Internet Explorer 9 dla systemu Windows 7 - wersja dla systemуw opartyc...

	.EXAMPLE
		Get list of updates without language packs and updatets that's not hidden.
	
		PS C:\> Get-WUList -NotCategory "Language packs"

		ComputerName Status KB          Size Title
		------------ ------ --          ---- -----
		G1           ------ KB2640148   8 MB Aktualizacja systemu Windows 7 dla komputerуw z procesorami x64 (KB2640148)
		G1           ------ KB2600217  32 MB Aktualizacja dla programu Microsoft .NET Framework 4 w systemach Windows XP, Se...
		G1           ------ KB2679255   6 MB Aktualizacja systemu Windows 7 dla komputerуw z procesorami x64 (KB2679255)
		...
		Foreach($Computer in $ComputerName)
		{
			If(Test-Connection -ComputerName $Computer -Quiet)
			{
				...
				Catch
				{
					...
					{
						Write-Error $_
					} #End Else $_ -match "HRESULT: 0x80072EE2"					
					Return
				} #End Catch
...
$downloaded = Get-WUList |ft
#$downloaded
$count = $downloaded.count

if($count -gt 0){Write-Host $count -ForegroundColor Green}else{
Write-Host $count -ForegroundColor Magenta
}
######
} 
